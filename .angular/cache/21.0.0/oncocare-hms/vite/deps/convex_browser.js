...
var LocalSyncState = class {
  ...
  restart() {
    ...
    const authenticate = __spreadValues({
      type: "Authenticate",
      baseVersion: 0
    }, this.auth);
  ...
  resume() {
    ...
    const authenticate = this.auth !== void 0 ? __spreadValues({
      type: "Authenticate",
...
function parseServerMessage(encoded) {
  switch (encoded.type) {
    ...
    case "Ping": {
      return __spreadValues({}, encoded);
    }
    case "MutationResponse": {
      if (encoded.success) {
        return __spreadProps(__spreadValues({}, encoded), { ts: u64ToLong(encoded.ts) });
      } else {
        return __spreadValues({}, encoded);
      }
    }
    case "Transition": {
      return __spreadProps(__spreadValues({}, encoded), {
        startVersion: __spreadProps(__spreadValues({}, encoded.startVersion), {
          ts: u64ToLong(encoded.startVersion.ts)
        }),
        endVersion: __spreadProps(__spreadValues({}, encoded.endVersion), {
  ...
  switch (message.type) {
    ...
    case "Connect": {
      if (message.maxObservedTimestamp !== void 0) {
        ...
        return __spreadProps(__spreadValues({}, message), { maxObservedTimestamp: void 0 });
      }
    }
    default: {
      message;
...
var WebSocketManager = class {
   ...
   */
  sendMessage(message) {
    const messageForLog = __spreadValues({
      type: message.type
  ...
  pause() {
    switch (this.socket.state) {
      ...
      case "connecting":
      case "ready": {
        this.socket = __spreadProps(__spreadValues({}, this.socket), { paused: "yes" });
        return;
      }
      default: {
        this.socket;
  ...
  resume() {
    switch (this.socket.state) {
      case "connecting":
        this.socket = __spreadProps(__spreadValues({}, this.socket), { paused: "no" });
        return;
      case "ready":
...
var AuthenticationManager = class {
  ...
  setConfig(fetchToken, onChange, onRefreshChange) {
    return __async(this, null, function* () {
      this.resetAuthState();
      this._logVerbose("pausing WS for auth token fetch");
      this.pauseSocket();
      const token = yield this.fetchTokenAndGuardAgainstRace(fetchToken, {
  ...
  tryToReauthenticate(serverMessage) {
    return __async(this, null, function* () {
      ...
      const token = yield this.fetchTokenAndGuardAgainstRace(
  ...
  fetchTokenAndGuardAgainstRace(fetchToken, fetchArgs) {
    return __async(this, null, function* () {
      const originalConfigVersion = ++this.configVersion;
      this._logVerbose(
        `fetching token with config version ${originalConfigVersion}`
      );
      const token = yield fetchToken(fetchArgs);
...
var BaseConvexClient = class {
    ...
    this.webSocketManager = new WebSocketManager(
      wsUri,
      {
        onOpen: (reconnectMetadata) => {
          ...
          this.webSocketManager.sendMessage(__spreadProps(__spreadValues({}, reconnectMetadata), {
            type: "Connect",
  ...
  mutation(name, args, options) {
    return __async(this, null, function* () {
      const result = yield this.mutationInternal(name, args, options);
      if (!result.success) {
        if (result.errorData !== void 0) {
          throw forwardData(
            result,
   ...
   * @internal
   */
  mutationInternal(udfPath, args, options, componentPath) {
    return __async(this, null, function* () {
  ...
  action(name, args) {
    return __async(this, null, function* () {
      const result = yield this.actionInternal(name, args);
      if (!result.success) {
        if (result.errorData !== void 0) {
          throw forwardData(
            result,
   ...
   * @internal
   */
  actionInternal(udfPath, args, componentPath) {
    return __async(this, null, function* () {
...
var PaginatedQueryClient = class {
  ...
  onBaseTransition(transition) {
    ...
    const extendedTransition = __spreadProps(__spreadValues({}, transition), {
      paginatedQueries
    });
  ...
  splitPaginatedQueryPage(paginatedQuery, pageKey, splitCursor, continueCursor) {
    ...
    const firstSubscription = this.client.subscribe(
      ...
      __spreadProps(__spreadValues({}, paginatedQuery.args), {
        paginationOpts: __spreadProps(__spreadValues({}, paginationOpts), {
          cursor: null,
  ...
  addPageToPaginatedQuery(token, continueCursor, numItems) {
    ...
    const pageArgs = __spreadProps(__spreadValues({}, paginatedQuery.args), {
      paginationOpts
    });
...
var ConvexClient = class {
  ...
  constructor(address, options = {}) {
    ...
    __publicField11(this, "_client");
    __publicField11(this, "_paginatedClient");
    __publicField11(this, "callNewListenersWithCurrentValuesTimer");
    __publicField11(this, "_closed");
    __publicField11(this, "_disabled");
    if (options.skipConvexDeploymentUrlCheck !== true) {
      validateDeploymentUrl(address);
    }
    const _a = options, { disabled } = _a, baseOptions = __objRest(_a, ["disabled"]);
    this._closed = false;
  ...
  mutation(mutation, args, options) {
    return __async(this, null, function* () {
      ...
      return yield this.client.mutation(getFunctionName(mutation), args, options);
    });
  }
  /**
   * Execute an action function.
  ...
  action(action, args) {
    return __async(this, null, function* () {
      if (this.disabled) throw new Error("ConvexClient is disabled");
      return yield this.client.action(getFunctionName(action), args);
    });
  }
  /**
...
var ConvexHttpClient = class {
   ...
   */
  consistentQuery(query, ...args) {
    return __async(this, null, function* () {
      const queryArgs = parseArgs(args[0]);
      const timestampPromise = this.getTimestamp();
      return yield this.queryInner(query, queryArgs, { timestampPromise });
    });
  }
  getTimestamp() {
    return __async(this, null, function* () {
  ...
  getTimestampInner() {
    return __async(this, null, function* () {
      ...
      const response = yield localFetch(`${this.address}/api/query_ts`, __spreadProps(__spreadValues({}, this.fetchOptions), {
        method: "POST",
        headers
      }));
      if (!response.ok) {
  ...
  queryInner(query, queryArgs, options) {
    return __async(this, null, function* () {
      const name = getFunctionName(query);
      ...
      const localFetch = this.fetch || specifiedFetch || fetch;
      const timestamp = options.timestampPromise ? yield options.timestampPromise : void 0;
      ...
      const response = yield localFetch(endpoint, __spreadProps(__spreadValues({}, this.fetchOptions), {
        body,
        method: "POST",
        headers
      }));
  ...
  mutationInner(mutation, mutationArgs) {
    return __async(this, null, function* () {
      ...
      const localFetch = this.fetch || specifiedFetch || fetch;
      const response = yield localFetch(`${this.address}/api/mutation`, __spreadProps(__spreadValues({}, this.fetchOptions), {
        body,
        method: "POST",
        headers
      }));
  ...
  processMutationQueue() {
    return __async(this, null, function* () {
      ...
      while (this.mutationQueue.length > 0) {
        ...
        try {
          const result = yield this.mutationInner(mutation, args);
          resolve(result);
        } catch (error) {
          reject(error);
        }
  ...
  mutation(mutation, ...args) {
    return __async(this, null, function* () {
      ...
      if (queued) {
        return yield this.enqueueMutation(mutation, mutationArgs);
      } else {
        return yield this.mutationInner(mutation, mutationArgs);
      }
    });
  }
  /**
  ...
  action(action, ...args) {
    return __async(this, null, function* () {
      ...
      const localFetch = this.fetch || specifiedFetch || fetch;
      const response = yield localFetch(`${this.address}/api/action`, __spreadProps(__spreadValues({}, this.fetchOptions), {
        body,
        method: "POST",
        headers
      }));
   ...
   * @internal
   */
  function(anyFunction, componentPath, ...args) {
    return __async(this, null, function* () {
      ...
      const localFetch = this.fetch || specifiedFetch || fetch;
      const response = yield localFetch(`${this.address}/api/function`, __spreadProps(__spreadValues({}, this.fetchOptions), {
        body,
        method: "POST",
        headers
      }));
...
//# sourceMappingURL=convex_browser.js.map
