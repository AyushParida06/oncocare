...
const info = {
  ...
  subtitle: 'Intelligent Imaging Operations',
  icon: 'ti-camera',
  glowColor: '96,165,250',
  features: [
    { icon: 'ti-ti-clipboard', label: 'Imaging Requests', desc: 'Digital radiology order management' },
    { icon: 'ti-ti-device-desktop', label: 'PACS Integration', desc: 'Image viewing and storage' },
    { icon: 'ti-ti-file-text', label: 'Report Generation', desc: 'Radiologist reporting tools' },
    { icon: 'ti-ti-radioactive', label: 'Dose Tracking', desc: 'Radiation dose monitoring' },
  ],
};
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      <div style={{ ...cardStyle, padding: '24px 28px', display: 'flex', alignItems: 'center', gap: 18, borderLeft: `4px solid rgb(${info.glowColor})` }}>
        ...
        <div>
          <div style={{ fontSize: 20, fontWeight: 700, color: textMain, ...(isDark ? { textShadow: `0 0 10px rgba(${info.glowColor},0.5)` } : {}) }}>{l('mod_ris')}</div>
