...
export default function Outpatient() {
  ...
  const inputBorderC = isDark ? '#555'  : '#e5e7eb';
  const rowBorder  = isDark ? '#3a3a3a' : '#f4f6f8';
  const mrnBg      = isDark ? '#333'    : '#f4f6f8';

  const cardStyle = { background: cardBg, border: `0.5px solid ${cardBorder}`, borderRadius: 12, boxShadow: isDark ? '0 0 12px rgba(29,158,117,0.15)' : 'none', transition: 'background 0.3s, border-color 0.3s' };
  ...
  const STATUS_COLORS = {
    ...
    discharged:{ bg: isDark ? 'rgba(170,170,170,0.15)' : '#f4f6f8', color: '#aaa'   },
  };
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        ...
        <button onClick={() => { setTab(tab === 'form' ? 'list' : 'form'); setSuccess(''); setError(''); }}
          style={{ background: tab === 'form' ? (isDark ? '#333' : '#f4f6f8') : '#0F6E56', color: tab === 'form' ? textMuted : '#fff', border: 'none', borderRadius: 8, padding: '8px 16px', fontSize: 13, fontWeight: 500, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 6, boxShadow: tab !== 'form' && isDark ? '0 0 10px rgba(29,158,117,0.5)' : 'none' }}>
      ...
      {tab === 'form' && (
        <form onSubmit={handleSubmit(onSubmit)} style={{ ...cardStyle, padding: 24 }}>
          ...
          <div style={{marginTop:20,display:'flex',gap:10}}>
            <button type="submit" disabled={saving} style={{background:saving?'#aaa':'#0F6E56',color:'#fff',border:'none',borderRadius:8,padding:'10px 24px',fontSize:14,fontWeight:500,cursor:saving?'not-allowed':'pointer',boxShadow:isDark&&!saving?'0 0 10px rgba(29,158,117,0.5)':'none'}}>
              {saving ? 'Saving…' : 'Register Patient'}
            </button>
            <button type="button" onClick={()=>{setTab('list');reset();}} style={{background:isDark?'#333':'#f4f6f8',color:textMuted,border:'none',borderRadius:8,padding:'10px 18px',fontSize:14,cursor:'pointer'}}>Cancel</button>
          </div>
        </form>
      )}
      ...
      {tab === 'list' && (
        <div style={{ ...cardStyle, overflow: 'hidden' }}>
          ...
          {patients === undefined && <div style={{ padding: 24, textAlign: 'center', color: textMuted, fontSize: 13 }}>Loading…</div>}
            ...
            return (
              <div key={p._id} style={{ padding: '12px 16px', borderBottom: i < patients.length - 1 ? `0.5px solid ${rowBorder}` : 'none', display: 'grid', gridTemplateColumns: '120px 1fr 1fr 140px 100px 100px 80px', gap: 12, alignItems: 'center' }}>
                ...
                <div>
                  ...
                  <div style={{ fontSize: 11, color: textMuted }}>{p.gender} · {p.phone}</div>
                </div>
