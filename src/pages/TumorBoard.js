    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20, height: 'calc(100vh - 100px)' }}>
      ...
      <div style={{
        ...
        <div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 8 }}>
            ...
            <div style={{ fontSize: 20, fontWeight: 700, color: isDark ? '#f5f5f5' : '#111' }}>{l('mod_tumor_board')}</div>
          </div>
      ...
      <div style={{ display: 'flex', gap: 20, flex: 1, minHeight: 0 }}>
        ...
        <div style={{
          ...
          <div style={{ padding: '20px 24px', borderBottom: `1px solid ${isDark ? '#333' : '#eee'}`, fontWeight: 600, color: isDark ? '#f5f5f5' : '#111', display: 'flex', alignItems: 'center', gap: 8 }}>
            <i className="ti ti-calendar-event" style={{ color: isDark ? '#f472b6' : '#db2777' }} />
            {l('tb_meetings')}
          </div>
          <div style={{ flex: 1, overflowY: 'auto', padding: '12px' }}>
            {meetings.length === 0 ? (
              <div style={{ padding: 20, textAlign: 'center', color: isDark ? '#666' : '#aaa', fontSize: 13 }}>No meetings scheduled.</div>
            ) : (
              meetings.map(m => {
                ...
                return (
                  <div 
                    ...
                    onMouseEnter={e => { if(!isSelected) e.currentTarget.style.background = isDark ? '#222' : '#f9fafb' }}
                    onMouseLeave={e => { if(!isSelected) e.currentTarget.style.background = 'transparent' }}
                  >
                    <div style={{ fontWeight: 600, color: isSelected ? (isDark ? '#f472b6' : '#db2777') : (isDark ? '#ddd' : '#333'), marginBottom: 4 }}>
          ...
          {selectedMeeting ? (
            <>
              <div style={{ padding: '20px 24px', borderBottom: `1px solid ${isDark ? '#333' : '#eee'}`, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                <div>
                  <div style={{ fontSize: 18, fontWeight: 600, color: isDark ? '#f5f5f5' : '#111', marginBottom: 4 }}>
              ...
              <div style={{ flex: 1, overflowY: 'auto', padding: 24 }}>
                <div style={{ fontSize: 15, fontWeight: 600, color: isDark ? '#ccc' : '#444', marginBottom: 16, display: 'flex', alignItems: 'center', gap: 8 }}>
                  <i className="ti ti-clipboard-list" />
                  {l('tb_cases')} ({meetingCases.length})
                </div>
                
                {meetingCases.length === 0 ? (
                  <div style={{ padding: 40, textAlign: 'center', color: isDark ? '#666' : '#aaa', fontSize: 14 }}>
                    ...
                    {meetingCases.map(c => (
                      <div key={c._id} style={{
                        ...
                        <div style={{ marginBottom: 16 }}>
                          <div style={{ fontSize: 12, fontWeight: 600, color: isDark ? '#aaa' : '#666', marginBottom: 4, textTransform: 'uppercase', letterSpacing: '0.05em' }}>{l('tb_summary')}</div>
                          <div style={{ fontSize: 14, color: isDark ? '#ddd' : '#333', lineHeight: 1.5, background: isDark ? '#1a1a1a' : '#fff', padding: 12, borderRadius: 8, border: `1px solid ${isDark ? '#333' : '#eee'}` }}>
                            {c.summary}
                          </div>
                        </div>

                        <div>
                          <div style={{ fontSize: 12, fontWeight: 600, color: isDark ? '#aaa' : '#666', marginBottom: 4, textTransform: 'uppercase', letterSpacing: '0.05em' }}>{l('tb_decision')}</div>
                          {decisionEditingId === c._id ? (
                            <div style={{ display: 'flex', gap: 10 }}>
                              <input 
                                ...
                                style={{ flex: 1, padding: 10, borderRadius: 8, border: `1px solid ${isDark ? '#444' : '#ddd'}`, background: isDark ? '#1a1a1a' : '#fff', color: isDark ? '#fff' : '#111', fontSize: 14, outline: 'none' }}
                            ...
                            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: isDark ? 'rgba(16,185,129,0.05)' : '#ecfdf5', padding: 12, borderRadius: 8, border: `1px solid ${isDark ? 'rgba(16,185,129,0.2)' : '#d1fae5'}` }}>
                              <div style={{ fontSize: 14, color: c.decision ? (isDark ? '#34d399' : '#059669') : (isDark ? '#888' : '#aaa'), fontWeight: c.decision ? 500 : 400 }}>
            ...
            <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', color: isDark ? '#666' : '#aaa', gap: 16 }}>
      ...
      {showMeetingModal && (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.6)', backdropFilter: 'blur(4px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}>
          <div style={{ background: isDark ? '#1a1a1a' : '#fff', border: `1px solid ${isDark ? '#333' : '#eee'}`, borderRadius: 16, width: '100%', maxWidth: 400, boxShadow: '0 20px 40px rgba(0,0,0,0.3)', overflow: 'hidden' }}>
            <div style={{ padding: '20px 24px', borderBottom: `1px solid ${isDark ? '#333' : '#eee'}`, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              <div style={{ fontSize: 18, fontWeight: 600, color: isDark ? '#fff' : '#111' }}>{l('tb_new_meeting')}</div>
              <button onClick={() => setShowMeetingModal(false)} style={{ background: 'none', border: 'none', color: isDark ? '#aaa' : '#666', cursor: 'pointer', fontSize: 20 }}>&times;</button>
            </div>
            <form onSubmit={handleAddMeeting} style={{ padding: 24, display: 'flex', flexDirection: 'column', gap: 16 }}>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                <label style={{ fontSize: 13, fontWeight: 500, color: isDark ? '#aaa' : '#555' }}>{l('tb_date')}</label>
                <input type="date" required value={meetingForm.date} onChange={e => setMeetingForm({...meetingForm, date: e.target.value})} style={{ padding: 12, borderRadius: 8, border: `1px solid ${isDark ? '#444' : '#ddd'}`, background: isDark ? '#222' : '#f9fafb', color: isDark ? '#fff' : '#111', fontSize: 14, outline: 'none' }} />
              </div>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                <label style={{ fontSize: 13, fontWeight: 500, color: isDark ? '#aaa' : '#555' }}>{l('tb_location')} (Optional)</label>
                <input type="text" placeholder="e.g. Conference Room B" value={meetingForm.location} onChange={e => setMeetingForm({...meetingForm, location: e.target.value})} style={{ padding: 12, borderRadius: 8, border: `1px solid ${isDark ? '#444' : '#ddd'}`, background: isDark ? '#222' : '#f9fafb', color: isDark ? '#fff' : '#111', fontSize: 14, outline: 'none' }} />
              </div>
              <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 10, marginTop: 10 }}>
                <button type="button" onClick={() => setShowMeetingModal(false)} style={{ padding: '10px 16px', borderRadius: 8, border: `1px solid ${isDark ? '#444' : '#ddd'}`, background: 'transparent', color: isDark ? '#aaa' : '#555', cursor: 'pointer', fontWeight: 500 }}>Cancel</button>
      ...
      {showCaseModal && (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.6)', backdropFilter: 'blur(4px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}>
          <div style={{ background: isDark ? '#1a1a1a' : '#fff', border: `1px solid ${isDark ? '#333' : '#eee'}`, borderRadius: 16, width: '100%', maxWidth: 500, boxShadow: '0 20px 40px rgba(0,0,0,0.3)', overflow: 'hidden' }}>
            <div style={{ padding: '20px 24px', borderBottom: `1px solid ${isDark ? '#333' : '#eee'}`, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              <div style={{ fontSize: 18, fontWeight: 600, color: isDark ? '#fff' : '#111' }}>{l('tb_add_case')}</div>
              <button onClick={() => setShowCaseModal(false)} style={{ background: 'none', border: 'none', color: isDark ? '#aaa' : '#666', cursor: 'pointer', fontSize: 20 }}>&times;</button>
            </div>
            <form onSubmit={handleAddCase} style={{ padding: 24, display: 'flex', flexDirection: 'column', gap: 16 }}>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                ...
                <select required value={caseForm.patientId} onChange={e => setCaseForm({...caseForm, patientId: e.target.value})} style={{ padding: 12, borderRadius: 8, border: `1px solid ${isDark ? '#444' : '#ddd'}`, background: isDark ? '#222' : '#f9fafb', color: isDark ? '#fff' : '#111', fontSize: 14, outline: 'none' }}>
                  <option value="">-- Choose Patient --</option>
                  {patients.map(p => <option key={p._id} value={p._id}>{p.firstName} {p.lastName} ({p.mrn})</option>)}
                </select>
              </div>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                <label style={{ fontSize: 13, fontWeight: 500, color: isDark ? '#aaa' : '#555' }}>{l('tb_summary')}</label>
                <textarea required rows={4} placeholder="Summarize the clinical presentation, histology, and question for the board..." value={caseForm.summary} onChange={e => setCaseForm({...caseForm, summary: e.target.value})} style={{ padding: 12, borderRadius: 8, border: `1px solid ${isDark ? '#444' : '#ddd'}`, background: isDark ? '#222' : '#f9fafb', color: isDark ? '#fff' : '#111', fontSize: 14, outline: 'none', resize: 'vertical' }} />
              </div>
              <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 10, marginTop: 10 }}>
                <button type="button" onClick={() => setShowCaseModal(false)} style={{ padding: '10px 16px', borderRadius: 8, border: `1px solid ${isDark ? '#444' : '#ddd'}`, background: 'transparent', color: isDark ? '#aaa' : '#555', cursor: 'pointer', fontWeight: 500 }}>Cancel</button>
