...
export async function getStats(ctx: QueryCtx | MutationCtx) {
  const existing = await ctx.db
    .query("stats")
    .withIndex("by_key", (q: any) => q.eq("key", "dashboard"))
    .unique();
...
export async function updatePatientStats(
  ctx: MutationCtx,
  diff: {
    total?: number;
    active?: number;
    remission?: number;
    typeChange?: { oldType?: string; newType?: string };
    admissions?: number;
  }
) {
  ...
  if (diff.typeChange) {
    if (diff.typeChange.oldType) {
      byType[diff.typeChange.oldType] = Math.max(
        0,
        (byType[diff.typeChange.oldType] || 0) - 1
      );
  ...
  await ctx.db.patch(stats._id, {
    ...
    remissionPatients: Math.max(
      0,
      stats.remissionPatients + (diff.remission || 0)
    ),
    byType,
    activeAdmissionsCount: Math.max(
      0,
      stats.activeAdmissionsCount + (diff.admissions || 0)
    ),
  });
}
